<template>
  <!-- 监控区域 -->
  <div id="video-content" class="video-content" v-visibility-change="change">
...
export default {
  ...
  computed: {},
  created() {
    console.log(this.$route,'$route 封装的组件')
...
        }else if(hidden === true){
    ...
    // 获取摄像头信息
    async videoPreview() {
      const { data: res } = await this.$http.get('/haikang/haikangCamera', { params: { company: this.parentId || '' } });
      if (res.data) {
        
        this.previewlist = res.data;
        console.log(' 获取摄像头信息结果++++',this.previewlist);
      }
      this.initPlugin();
    },
    // 获取海康配置信息
    ...
    initPlugin() {
      this.oWebControl = new WebControl({
        ...
        cbConnectClose: (bNormalClose) => {
          console.log('插件使用过程中发生的断开与插件服务连接', bNormalClose);
    ...
    callbackmes() {
      this.oWebControl.JS_SetWindowControlCallback({
        cbIntegrationCallBack: function (oData) {
          ...
          // console.log('批量预览成功后的消息回调',JSON.stringify(oData)); // 打印消息体至控制台
        },
      });
    },
