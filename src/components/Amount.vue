<template>
  <div class="com-container">
    <div class="amount-chart" ref="amount_ref"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initChart() {
      ...
      this.chartInstance = this.$echarts.init(this.$refs.amount_ref); // 初始化实例对象
      // 指定图表的配置项和数据
      const initOption = {
        xAxis: {
          type: 'category',
        ...
        series: [
          {
            // data: [150, 130, 120, 110, 100, 70, 40, 20, 80, 70, 50, 40, 10],
            type: 'bar',
            barWidth: 15,
            itemStyle: {
              normal: {
    ...
    // 获取数据
    async getData() {
      const {data:res} = await this.$http.get('system/project/projectCntForJu',{params:{parentId:100}});
      console.log('部门项目数量结果', res);
    ...
    updateChart() {
      ...
      this.allData.map(item=>{
        // console.log('部门项目数量',item);
        xdata.push(item.deptName)
        ydata.push(item.projectCnt)
      })
      // 数据配置项
      const dataOption = {
        xAxis: {
    ...
    // 监听窗口变化
    screenAdapter() {
       const titleFontSize = (this.$refs.amount_ref.offsetWidth / 100) * 3.6;
      const adapterOption = {
         xAxis: {
          axisLabel: {
