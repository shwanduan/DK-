  ...
  <div class="com-container">
    <!-- <div class="right-bottom-content-left">
      <div class="content-left" v-for="(item, index) in allData" :key="index">
        <span>{{ fillZero(index + 1) }}:{{ item.deptName }}</span>
      </div>
    </div> -->
...
export default {
  ...
  methods: {
    ...
    // 获取数据
    async getData() {
      const { data: res } = await this.$http.get('system/project/projectCntForYuan', { params: { company: this.parentId } });
      this.allData = res.data;
      console.log('部门人数----------》', this.allData);
      this.updateChart();
    },
    // 数据跟新
    updateChart() {
      ...
      this.allData.map((item, index) => {
        ...
        xdata.push(parseInt(index + 1)+'-'+item.deptName)
        ydata.push({
          value: item.projectCnt,
          name: item.deptName,
        });
      });
